...
local saveDir="$SCRIPT_DIR/../CovidLevels/Assets.xcassets/AppIcon.appiconset"
#local saveDir="$SCRIPT_DIR/test"
...
function append_export_command () {
    ...
    printf "export-filename: $saveDir/AppIcon-${idiom}-${height}x${height}@${scale}x.png;" >> $commandFile
    printf "export-height: $(($height * $scale));" >> $commandFile
    printf "export-background-opacity: 255;" >> $commandFile
    printf "export-do;" >> $commandFile
}
